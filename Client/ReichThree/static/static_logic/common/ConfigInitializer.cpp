...
extern std::string g_currLanuage;
...
{
	LocalizationCfg::getInstance().SetCurrLanguage(g_currLanuage);
...
bool ConfigInitializer::LoadConfig()
{
	...
	if (!DonateCfg::getInstance().initialize()) return false;
	if (!FoodCfg::getInstance().initialize()) return false;
	if (!ServerListCfg::getInstance().initialize()) return false;
	return true;
}
bool ConfigInitializer::UnloadConfig()
{
	...
	DonateCfg::getInstance().terminate();
	FoodCfg::getInstance().terminate();
	ServerListCfg::getInstance().terminate();
	return true;
}
