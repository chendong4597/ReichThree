...
//			BUFF_EFFECT_TYPE emDotType = (BUFF_EFFECT_TYPE)BuffEffect.type;
	...
	class TSkillBuffStrategy_Restore final : public TSkillBuffStrategy<TSkillEventHandler>
	{
		virtual void BuffUpdate(SkillBuffInfo& pBuffInfo, NS::uint EffectIndex, TSkillEventHandler* /*pEventHandler*/) override
		{
			...
			auto pGameObject = HandlerRole::findRole(pBuffInfo.OwnerMapObjectID);
			if (!pGameObject) return;
			auto&& pRoleTarget = dynamic_cast<Role*>(pGameObject);
			if (!pRoleTarget) return;
			...
			param.TargetRoleValue.GetSkillRoleValue_ByCharacter(param.pTarget);
			param.theEffect.values = BuffEffect.values;
			if (param.theEffect.values.size() <= 0) return;

			int restoreValue;
			//SkillEffect_Restore().CalEffect(param, restoreValue);

			//// 最终回复值会在90%到100%之间随机
			//static_cast<int>(static_cast<float>(restoreValue) * NS::RandomUtil::getRandomFloat(0.9f, 1.0f));
			//// 執行
			...
			{
				restoreValue = static_cast<int>(BuffEffect.values[0].value * 1.f * pRoleTarget->getMaxHp() / 100);
				int nCurHP = pRoleTarget->getHp();
				int nMaxHp = pRoleTarget->getMaxHp();
				//// CLV - 8876【药剂】满蓝，使用回蓝药剂，不应该显示加多少蓝
				//// 如果已经满血/满蓝了，在Buff中添加处理，不需要再下发加血/加蓝消息，客户端修改完毕之后，请再转服务端处理
				if (nCurHP >= nMaxHp || nCurHP <= 0)
				{
					return;
				}
			...
			{
				// CLV - 8876【药剂】满蓝，使用回蓝药剂，不应该显示加多少蓝
				restoreValue = static_cast<int>(BuffEffect.values[0].value * 1.f * pRoleTarget->getMaxMp() / 100);
				int nCurMP = pRoleTarget->getMp();
				int nMaxMp = pRoleTarget->getMaxMp();
				//// 如果已经满血/满蓝了，在Buff中添加处理，不需要再下发加血/加蓝消息，客户端修改完毕之后，请再转服务端处理
				if (nCurMP >= nMaxMp)
				{
					return;
