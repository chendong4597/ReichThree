...
std::vector<std::pair<NS::I64, NS::I64>> g_vecKillMobs;
float g_killstm = 0.f;
...
LocalServer_Battle::LocalServer_Battle(void)
{
	...
	HOOK_MSG(onMsgSkillEffectDotRestoreMS2C);
...
void LocalServer_Battle::onMsgSkillEffectDotRestoreMS2C(const MsgSkillEffectDotRestoreMS2C& msgRet)
{
	std::shared_ptr<MapObj> pCasterObject = MapObjMgr::getInstance().findObj(msgRet.target_mapobjectid());//HandlerRole::findRole(target_id);
	if (!pCasterObject) return;

	auto pCasterData = std::dynamic_pointer_cast<Role>(pCasterObject);//std::make_shared<Role>(pGameObject);
	if (!pCasterData || pCasterData->getHp() <= 0 ||
		pCasterData->getMaxHp() <= 0) {
		return;
	}
	...
	if (msgRet.hp_val() > 0) {
		MsgSkillEffectRestoreMS2C msg;
		msg.set_serialid(0);
		msg.set_caster_mapobjectid(msgRet.caster_mapobjectid());
		msg.set_target_mapobjectid(msgRet.target_mapobjectid());
		msg.set_skill_id(0);
		msg.set_hp_val(msgRet.hp_val());
		int nCurHP = pCasterData->getHp() + msgRet.hp_val();
		int nMaxHP = pCasterData->getMaxHp();
		if (nCurHP > nMaxHP)
		{
			nCurHP = nMaxHP;
		...
		pCasterData->setHp(nCurHP);
		msg.set_currhp(msgRet.hp_val());
		send(msg);
	}
	else if(msgRet.mp_val()) {
		int nCurMP = pCasterData->getMp() + msgRet.hp_val();
		int nMaxMP = pCasterData->getMaxMp();
		if (nCurMP > nMaxMP)
		{
			nCurMP = nMaxMP;
		}
...
void LocalServer_Battle::kill(int32 target_id, int32 caster_id, bool isTargetDead)
{
	...
	msg.set_casterobjectid(caster_id);
	send(msg);
	...
	if (i64IggId > 0 )
	{
		...
		if (pMap) {
			if (pMap->type == (int)MapDataMgr::MapType::Dungeon
				&& pMap->dungeontype == (int)MapSettingCfg::Advture_Dungeon
				&& pMap->id < 400000) {
				g_vecKillMobs.push_back(std::make_pair(caster_id, target_id));
				return;
			}
		}
...
void LocalServer_Battle::update(float dt)
{
	...
	if (g_vecKillMobs.size() <= 0) return;

	g_killstm += dt;
	if (g_killstm >= 1.0f) {
		if (auto pSelf = PlayerMgr::getInstance().getSelf())
		{
			NS::I64 iggId = pSelf->getIggId();
			MsgReportKillMutiMobRequest reportMsg;
			reportMsg.set_iggid(iggId);
			for (auto&& iter : g_vecKillMobs)
			{
				auto&& killmob = reportMsg.add_kiil_mobs();
				if (killmob) {
					killmob->set_map_type_id(iter.first);
					killmob->set_mob_type_id(iter.second);
				}
			}
			MsgMgr::getInstance().sendMessageDirect(_MSG_REPORT_KILL_MUTI_MOB, &reportMsg);
			g_vecKillMobs.clear();
		}
		g_killstm = 0.f;
	}
