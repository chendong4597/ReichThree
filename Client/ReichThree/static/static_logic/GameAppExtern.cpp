...
std::string g_currLanuage = "trunk";
...
void GameAppExtern::update()
{
	if (g_pFramework) {
		...
		LocalServer_Battle::getInstance().update(g_pFramework->getRealFrameTime());
	}
}
...
void GameAppExtern::setCurrLanguage(const char* str)
{
	if (str) {
		g_currLanuage = str;
	}
...
void GameAppExtern::setCurrServer(const char* strIp)
{
	if (strIp) {
		HandlerLogin::getInstance().selectServer(strIp);
	}
}

void GameAppExtern::resetBuffs()
{
	auto&& player = PlayerMgr::getInstance().getSelf();
	if (player) {
		LocalServer_Buff::getInstance().delAllBuff(player->getId());
	}

	auto&& npcs = NpcMgr::getInstance().getNpcs();
	for (auto&& npc : npcs)
	{
		LocalServer_Buff::getInstance().delAllBuff(npc.second->getId());
	}

	auto&& mobs = MonsterDataMgr::getInstance().getAllMonsters();
	for (auto&& mob : mobs)
	{
		LocalServer_Buff::getInstance().delAllBuff(mob.second->getId());
	}

	auto&& guards = GuardDataMgr::getInstance().getAllGurads();
	for (auto&& guard : guards)
	{
		LocalServer_Buff::getInstance().delAllBuff(guard.second->getId());
	}
}
...
{
	if (MapDataMgr::getInstance().GetSurvivalUuid() != 0) {
		GameAppExtern::ShowToastTips(_LC("UI_TIPS_IS_IN_SUR"));
		return false;
	}
...
void GameAppExtern::SetResetIggId(NS::I64 iggId)
{
	HandlerLogin::getInstance().setResetIggId(iggId);
}
