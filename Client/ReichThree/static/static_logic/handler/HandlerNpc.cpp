...
bool HandlerNpc::initialize()
{
	...
	REGISTER_S2C(onMsgProtectorUpgradeStarS2C);

	REGISTER_S2C(onMsgProtectorTradeS2C);
	REGISTER_S2C(onMsgProtectorTradeRecallS2C);
	REGISTER_S2C(onMsgProtectorTradeInfoS2C);
	REGISTER_S2C(onMsgProtectorTradeBuyS2C);
	REGISTER_S2C(onMsgProtectorTradeSellS2C);
	REGISTER_S2C(onMsgTradeNpcAppearance);
	REGISTER_S2C(onMsgProtectorTradePriceGetS2C);
	REGISTER_S2C(onMsgDunReviveNpcS2C);
...
void HandlerNpc::onMsgProtectorUpgradeStarS2C(const MsgProtectorUpgradeStarS2C& msg)
{
	switch (msg.result())
	{
	case MsgProtectorUpgradeStarS2C_RESULT_FAILURE:
	{
		GameAppExtern::ShowToastTips(_LC("UI_NPC_UPGRADE_FAILED"));
	}
	break;
	case MsgProtectorUpgradeStarS2C_RESULT_SUCCESS:
	{
		auto&& pNpc = NpcMgr::getInstance().getNpcByProtectorUuid(msg.uuid());
		if (pNpc) {
			pNpc->calcAttr();
		}
		GameAppExtern::ShowToastTips(_LC("UI_NPC_UPGRADE_SUCCESS"));
		POST_DATA_EVENT(msg);
	}
	break;
	default:
...
void HandlerNpc::onMsgDunReviveNpcS2C(const MsgPB::MsgDunReviveNpcS2C& msg)
{
	POST_DATA_EVENT(msg);
}
...
void HandlerNpc::onMsgProtectorTradeS2C(const MsgPB::MsgProtectorTradeS2C& msg)
{
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_SellBuy;
	evt.nResult = msg.result();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgProtectorTradeRecallS2C(const MsgPB::MsgProtectorTradeRecallS2C& msg)
{
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_Recall;
	evt.nResult = msg.result();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgProtectorTradeInfoS2C(const MsgPB::MsgProtectorTradeInfoS2C& msg)
{
	NpcProtectorDataMgr::getInstance()->updateProtectorTradeData(msg);
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_Info;
	evt.iggid = msg.iggid();
	evt.uuid = msg.uuid();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgProtectorTradeBuyS2C(const MsgPB::MsgProtectorTradeBuyS2C& msg)
{
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_My_Buy;
	evt.nResult = msg.result();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgProtectorTradeSellS2C(const MsgPB::MsgProtectorTradeSellS2C& msg)
{
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_My_Sell;
	evt.nResult = msg.result();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgProtectorTradePriceGetS2C(const MsgPB::MsgProtectorTradePriceGetS2C& msg)
{
	NpcProtectorDataMgr::getInstance()->SetGuidPrice(msg.itemid(), msg.coin(), msg.cash());
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_GuidPrice;
	evt.nItemId = msg.itemid();
	POST_DATA_EVENT(evt);
}

void HandlerNpc::onMsgTradeNpcAppearance(const MsgPB::MsgTradeNpcAppearance& msg)
{
	NpcProtectorDataMgr::getInstance()->updateNpcTradeData(msg);
	HandlerTradeEvent evt;
	evt.m_type = HandlerTradeEvent::EventType::EventType_UpdateTrades;
	POST_DATA_EVENT(evt);
}
