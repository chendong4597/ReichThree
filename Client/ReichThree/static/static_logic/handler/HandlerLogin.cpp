...
static std::string s_str_server_ip = "47.111.109.210";
...
void HandlerLogin::selectServer(std::string strIP)
{
	s_str_server_ip = strIP;
}
...
void HandlerLogin::checkPlatformAccount(std::string strAccount, std::string strPwd, int platform, int client_ver)
{
	if (!m_bLoginToGs) {
		...
		m_strPwd = strPwd;
		m_platform = platform;
		m_client_ver = client_ver;
		NetworkModule::getInstance().connectToServer(s_str_server_ip.c_str(), GlobalCfg::getInstance().GetLoginPort());
		return;
	}
}
...
void HandlerLogin::onMsgLoginValidate(const MsgPB::MsgLoginValidate& msg)
{
	...
	m_bLoginToGs = true;
	NetworkModule::getInstance().connectToServer(s_str_server_ip.c_str(), m_port);
...
{
	if (msg.iggid() == 0 || msg.platform() < 0 || msg.account().empty())
	{
		POST_DATA_EVENT(msg);
		return;
	}
	...
	GameAppExtern::Login(msg.iggid());
	//if (m_nResetIggId > 1000000) {
	//	GameAppExtern::Login(m_nResetIggId);
	//}
	//else {
	//	GameAppExtern::Login(1001772);//msg.iggid());//
	//}
}
...
void HandlerLogin::onMsgConnectionState(const MsgPB::MsgConnectionState& msg)
{
	...
	{
		MsgMgr::getInstance().setConnectState(NetworkModule::ConnectState::Connecting);
	...
	case CS_CONNECTED:	// Connected.
	{
		...
		if (!m_bLoginToGs){
			...
			msg.set_account(m_strAccount);
			msg.set_pwd(m_strPwd);
			msg.set_platform(m_platform);
			msg.set_client_ver(m_client_ver);
			MsgMgr::getInstance().sendMessageDirect(msg);
		}
		else {
			loginToGS();
		}
		MsgMgr::getInstance().setConnectState(NetworkModule::ConnectState::Connected);
	}
	break;
	...
	case CS_FAILED:
		// Fall through.
		{
			...
			MsgMgr::getInstance().setConnectState(NetworkModule::ConnectState::Disconnect);
		}
		break;
	default:
		break;
