...
#include "config/DonateCfg.h"
#include "HandlerLogin.h"
#include "data/ItemData.h"
#include "data/NotesData.h"
#include "data/RankData.h"
...
bool HandlerRole::initialize()
{
	...
	REGISTER_S2C(onMsgPassiveSkillEffectDmgMS2C);
	...
	//捐献
	REGISTER_S2C(onMsgDonateS2C);

	REGISTER_S2C(onMsgLotteryS2C);

	REGISTER_S2C(onMsgRankS2C);

	REGISTER_S2C(onMsgArmyAssistS2C);

	//押镖
	REGISTER_S2C(onMsgConvoyS2C);
	REGISTER_S2C(onMsgConvoyRecallS2C);
	REGISTER_S2C(onMsgConvoyListS2C);
	REGISTER_S2C(onMsgConvoyRobberyS2C);
	return true;
}
...
void HandlerRole::onMsgPlayerInfo(const MsgPB::MsgPlayerInfoGS2C& msg)
{
	...
	if (msg.has_donatenum())
	{
		player->setDolnateNum(msg.donatenum());
	}

	if (msg.has_donatelv())
	{
		if (!isOtherPlayer) {
			int nPreDonaLv = player->getDolnateLv();
			player->setDolnateLv(msg.donatelv());
			
			if (nPreDonaLv != msg.donatelv()) {
				std::string strName = DonateCfg::getInstance().getDonateName(msg.donatelv());
				std::string str = fmt::format("{0} X {1}", _LC("UI_DONATE_SHOW"), _LC(strName.c_str()));
				GameAppExtern::ShowToastTips(str, static_cast<int>(HandlerTipsEvent::EventType::Unknown));
				player->calcAttr();
			}
		}
		else {
	...
	if (msg.has_data_flag()) {
		if (msg.data_flag() & MsgPlayerInfoGS2C_DATA_FLAG_DATA_FLAG_GEN) {
			player->ClearGenBuff();
			for (int i = 0; i < msg.genbuff_size(); i++)
			{
				player->addGenBuff(msg.genbuff(i).id(), msg.genbuff(i).endtime());
			}
		}

		if (msg.data_flag() & MsgPlayerInfoGS2C_DATA_FLAG_DATA_FLAG_FOOD) {
			player->ClearFoodBuff();
			for (int i = 0; i < msg.foodbuff_size(); i++)
			{
				player->addFoodBuff(msg.foodbuff(i).id(), msg.foodbuff(i).endtime());
			}
		}
	}
...
void HandlerRole::onMsgPassiveSkillEffectDmgMS2C(const MsgPB::MsgPassiveSkillEffectDmgMS2C& msg)
{
	POST_DATA_EVENT(msg);
}
...
void HandlerRole::onMsgDonateS2C(const MsgPB::MsgDonateS2C& msg)
{
	POST_DATA_EVENT(msg);
}

void HandlerRole::onMsgRankS2C(const MsgPB::MsgRankS2C& msg)
{
	RankDataMgr::getInstance().Clear();
	for (size_t i = 0; i < msg.data_size(); i++)
	{
		RankDataMgr::getInstance().AddDonateRank(msg.data(i).iggid(), msg.data(i).n64value(), msg.data(i).rankidx());
	}
	HandlerRankEvent evt;
	POST_DATA_EVENT(evt);
}

void HandlerRole::onMsgArmyAssistS2C(const MsgPB::MsgArmyAssistS2C& msg)
{
	POST_DATA_EVENT(msg);
}
...
void HandlerRole::onMsgConvoyS2C(const MsgPB::MsgConvoyS2C& msg)
{
	if (msg.result() == MsgConvoyS2C_RESULT_SUCCESS) {
		GameAppExtern::ShowToastTips(_LC("MsgConvoyS2C_Result_suc"), static_cast<int>(HandlerTipsEvent::EventType::Evt_Coin));
	}
}

void HandlerRole::onMsgConvoyRecallS2C(const MsgPB::MsgConvoyRecallS2C& msg)
{
	if (msg.result() == MsgConvoyRecallS2C_RESULT_SUCCESS) {
		GameAppExtern::ShowToastTips(_LC("MsgConvoyS2C_recall_suc"), static_cast<int>(HandlerTipsEvent::EventType::Evt_Coin));
	}
}

void HandlerRole::onMsgConvoyListS2C(const MsgPB::MsgConvoyListS2C& msg)
{
	ConvoyDataMgr::getInstance().RefreshConvoy(msg);
	HandlerConvoyEvent evt;
	evt.m_type = HandlerConvoyEvent::EventType::EventType_Convoy_List;
	POST_DATA_EVENT(evt);
}

void HandlerRole::onMsgConvoyRobberyS2C(const MsgPB::MsgConvoyRobberyS2C& msg)
{
	if (msg.result() == MsgConvoyRobberyS2C_RESULT_SUCCESS) {
		ConvoyDataMgr::getInstance().UpdateCurrConvoy(msg);
	}
	HandlerConvoyEvent evt;
	evt.m_type = HandlerConvoyEvent::EventType::EventType_Convoy_Bat;
	evt.nResult = msg.result();
	POST_DATA_EVENT(evt);
}
