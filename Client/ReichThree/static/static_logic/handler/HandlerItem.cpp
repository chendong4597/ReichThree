...
bool HandlerItem::initialize()
{
	...
	REGISTER_S2C(onMsgItemEquipAttrWashS2C);
	REGISTER_S2C(onMsgItemEquipAttrSaveS2C);
	return true;
}
...
void HandlerItem::updateItemData(const MsgPB::MsgItemDataGS2C_ItemData &msg, bool bAdd)
{
	...
	for (int i = 0; i < msg.tempbase_size(); i++)
	{
		pItem->addAttrEffect(AttrType::ATTR_BASE_TEMP, msg.tempbase(i));
	}

	for (int i = 0; i < msg.tempextattr_size(); i++)
	{
		pItem->addAttrEffect(AttrType::ATTR_EFFECT_TEMP, msg.tempextattr(i));
	}

	if (bFind && pItem->getPacketType() == PROTECTOR_EQUIP) {
		auto&& npcs = NpcMgr::getInstance().getNpcs();
		for (auto&& npc : npcs)
		{
			auto&& npcData = NpcProtectorDataMgr::getInstance()->getNpcProtectorDataByUuid(npc.second->GetProtectorUuid());
			if (npcData && (npcData->equipId == msg.uuid() ||
				npcData->weaponId1 == msg.uuid() || npcData->weaponId2 == msg.uuid())) {
				npc.second->calcAttr();
				break;
			}
		}
...
void HandlerItem::onMsgItemEquipAttrWashS2C(const MsgPB::MsgItemEquipAttrWashS2C& msg)
{
	if (msg.eres() != ITEM_WASH_SUCCES) {
		GameAppExtern::ShowToastTips(_LC("UI_WASH_ITEM_FAILED"));
	}
	else {
		GameAppExtern::ShowToastTips(_LC("UI_WASH_ITEM_SUC"));
	}
	POST_DATA_EVENT(msg);
}

void HandlerItem::onMsgItemEquipAttrSaveS2C(const MsgPB::MsgItemEquipAttrSaveS2C& msg)
{
	GameAppExtern::ShowToastTips(_LC("UI_WASH_ITEM_SELECT"));
	POST_DATA_EVENT(msg);
}
