...
void Player::CaluDonateAttr(AttrMap& mapAttr)
{
	auto&& pMapAttr = DonateCfg::getInstance().getDonateInfoById(getDolnateLv());
	if (pMapAttr) {
		mapAttr[ROLE_EFFECT_HPM] += pMapAttr->n_HPM;
		mapAttr[ROLE_EFFECT_MPM] += pMapAttr->n_MPM;
		mapAttr[ROLE_EFFECT_AD] += pMapAttr->n_AD;
		mapAttr[ROLE_EFFECT_AP] += pMapAttr->n_AP;
		mapAttr[ROLE_EFFECT_ARM] += pMapAttr->n_ARM;
		mapAttr[ROLE_EFFECT_MARM] += pMapAttr->n_MARM;
	}
}

void Player::CaluFoodAttr(AttrMap& mapAttr)
{
	m_foodAddSpeed = 0;
	FoodbuffMap::iterator iter = m_foodBuff.begin();
	for (; iter != m_foodBuff.end(); iter++)
	{
		auto&& pCfg = FoodCfg::getInstance().findFood(iter->first);
		if(!pCfg || pCfg->n_AddAttr == 0) continue;
		mapAttr[ROLE_EFFECT_HPM] += pCfg->n_HPM;
		mapAttr[ROLE_EFFECT_HPR] += pCfg->n_HPR;
		mapAttr[ROLE_EFFECT_MPM] += pCfg->n_MPM;
		mapAttr[ROLE_EFFECT_MPR] += pCfg->n_MPR;
		mapAttr[ROLE_EFFECT_AD] += pCfg->n_AD;
		mapAttr[ROLE_EFFECT_AP] += pCfg->n_AP;
		mapAttr[ROLE_EFFECT_ARM] += pCfg->n_ARM;
		mapAttr[ROLE_EFFECT_MARM] += pCfg->n_MARM;
		m_foodAddSpeed += pCfg->n_Speed;
	}
}
...
void Player::calcAttr(bool bInherBuff)
{
	...
	CaluDonateAttr(baseAttr);
	CaluFoodAttr(baseAttr);
...
void Player::addGenBuff(int nIdx, NS::I64 tmEnd)
{
	m_genBuf[nIdx] = tmEnd;
}

void Player::ClearFoodBuff()
{
	m_foodBuff.clear();
}

void Player::addFoodBuff(int nFoodItem, NS::I64 tmEnd)
{
	m_foodBuff[nFoodItem] = tmEnd;
}

NS::I64 Player::getGenbuff(int nGenIdx)
{
	auto iter = m_genBuf.find(nGenIdx);
	if (iter == m_genBuf.end()) {
		return 0;
	}
	return iter->second;
}
