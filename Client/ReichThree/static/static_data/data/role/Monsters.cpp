...
void Monster::calcAttr(bool bInherBuff)
{
	...
	//士兵作为怪物类，继承父将的武器 但是押镖要对等，押镖对方的士兵无法计算武器
	if (m_npcUuid != 0 && !(pMapCfg && pMapCfg->dungeontype == MapSettingCfg::Convoy_Dungeon))   
	{
...
std::shared_ptr<Monster> MonsterDataMgr::getMonsterByTowerUuid(bzbee::I64 tUuid)
{
	if (tUuid == 0) return nullptr;
	for (auto&& pir : m_monsters)
	{
		if (pir.second->getTowerDefenceUuid() == tUuid) {
			return pir.second;
		}
	}
