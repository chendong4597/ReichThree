...
ItemPacketMgr::WeakItemDataKeyPosMap ItemPacketMgr::findPacketMap(MsgPB::PACKET_TYPE packetType)
{
	...
	for (auto& it : m_itemDataMap)
	{
		...
		int nPos = it.second->getPos();
		if (packetType == MsgPB::PACKET_TYPE::PROTECTOR_EQUIP) {
			nPos = nPosIdx++;
		}
		itemDataMap.insert(std::make_pair(nPos, it.second));
	}
...
{
	if (lhs->m_itemId != rhs->m_itemId && lhs->getArmorCfg() && rhs->getArmorCfg()) {
		if (lhs->getArmorCfg()->m_equipType != rhs->getArmorCfg()->m_equipType)
			return lhs->getArmorCfg()->m_equipType < rhs->getArmorCfg()->m_equipType;
		else if (lhs->getArmorCfg()->m_equipSubType != rhs->getArmorCfg()->m_equipSubType) {
			return lhs->getArmorCfg()->m_equipSubType < rhs->getArmorCfg()->m_equipSubType;
		} else if (lhs->getArmorCfg()->m_equipPos != rhs->getArmorCfg()->m_equipPos) {
			return lhs->getArmorCfg()->m_equipPos < rhs->getArmorCfg()->m_equipPos;
		}
	}
