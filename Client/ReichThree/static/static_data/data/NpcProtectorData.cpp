...
{
	if (MsgPB::PROTECTOR_STATUS::BADGE_STATUS_PATROL != status &&
		MsgPB::PROTECTOR_STATUS::BADGE_STATUS_ENABLE_FUNCTION != status) {
		return false;
	}
...
bool NpcProtectorDataMgr::isHasGeneralDead()
{
	auto npcList = getProtectorDataMap();
	auto&& iter = npcList.begin();
	for (; iter != npcList.end(); iter++)
	{
		if (!iter->second || iter->second->uuid == 0)
		{
			continue;
		}

		auto&& pNpcType = NpcTypeCfg::getInstance().GetNpcType(iter->second->itemid);
		if (!pNpcType) {
			continue;
		}

		if (pNpcType->n_Type != NpcTypeCfg::NpcType_Genels) {
			continue;
		}

		if (iter->second->status == MsgPB::PROTECTOR_STATUS::BADGE_STATUS_DEATH)
		{
			return true;
		}
	}
...
bool NpcProtectorDataMgr::isHasComfelRedByUuid(NS::I64 uuid)
{
	auto&& data = getNpcProtectorDataByUuid(uuid);
	if (!data) return false;

	auto&& npcType = NpcTypeCfg::getInstance().GetNpcType(data->itemid);
	if (!npcType) return false;

	if (npcType->n_Type != NpcTypeCfg::NpcType_Genels) return false;
	auto&& pMapCfg = MapSettingCfg::getInstance().findMap(MapDataMgr::getInstance().getMapId());
	if (pMapCfg && pMapCfg->type == 3)
	{
		if (NpcMgr::getInstance().getNpcByProtectorUuid(uuid) == nullptr) {
			return false;
		}
	}
	int nItemId = 0;
	...
	if (data->comfort == 0) {
		nItemId = 1421101;
		nNeedNum = npcType->n_Piece1;
	}
	else if (data->comfort == 1) {
		nItemId = 1421102;
		nNeedNum = npcType->n_Piece2;
	}
	if (nItemId == 0 || nNeedNum == 0) return false;
	int nHasNum = ItemPacketMgr::getInstance().getPacketItemCount(BODY, nItemId);
	return nHasNum >= nNeedNum;
}

bool NpcProtectorDataMgr::isHasComfelRed()
{
	auto npcList = getProtectorDataMap();
	auto&& iter = npcList.begin();
	for (; iter != npcList.end(); iter++)
	{
		...
		if (isHasComfelRedByUuid(iter->second->uuid)) {
			return true;
		}
	}
	return false;
...
void NpcProtectorDataMgr::updateProtectorTradeData(const MsgPB::MsgProtectorTradeInfoS2C& msg)
{
	m_TradeProtectorData.buyItems.clear();
	m_TradeProtectorData.sellItems.clear();
	m_TradeProtectorData.iggId = msg.tradeiggid();
	m_TradeProtectorData.uuid = msg.uuid();
	m_TradeProtectorData.itemId = msg.itemid();
	for (int i = 0; i < msg.sellitems_size(); i++) {
		auto&& item = msg.sellitems(i);
		ProtectorTrade data;
		data.nItemId = item.itemid();
		data.nNum = item.count();
		data.nPrice = item.price();
		data.nPriceTpy = item.pricetpy();
		data.nCurNum = item.curnum();
		m_TradeProtectorData.sellItems.push_back(data);
	}
	for (int i = 0; i < msg.buyitems_size(); i++) {
		auto&& item = msg.buyitems(i);
		ProtectorTrade data;
		data.nItemId = item.itemid();
		data.nNum = item.count();
		data.nPrice = item.price();
		data.nPriceTpy = item.pricetpy();
		data.nCurNum = item.curnum();
		m_TradeProtectorData.buyItems.push_back(data);
	}
}

void NpcProtectorDataMgr::updateNpcTradeData(const MsgPB::MsgTradeNpcAppearance& msg)
{
	m_vecSellsData.clear();
	m_vecBuysData.clear();
	for (int i = 0; i < msg.npcselldata_size(); i++)
	{
		auto&& data = msg.npcselldata(i);
		NpcTradeData info;
		info.uuid = data.tradeuuid();
		info.iggid = data.tradeiggid();
		m_vecSellsData.push_back(info);
	}
	for (int i = 0; i < msg.npcbuydata_size(); i++)
	{
		auto&& data = msg.npcbuydata(i);
		NpcTradeData info;
		info.uuid = data.tradeuuid();
		info.iggid = data.tradeiggid();
		m_vecBuysData.push_back(info);
	}
}

const NpcTradeData* NpcProtectorDataMgr::GetTradeNpcData(int nIdx, bool bSell)
{
	if (bSell) {
		if (nIdx >= m_vecSellsData.size()) {
			return nullptr;
		}
		return &(m_vecSellsData[nIdx]);
	}
	else {
		if (nIdx >= m_vecBuysData.size()) {
			return nullptr;
		}
		return &(m_vecBuysData[nIdx]);
	}
}


int NpcProtectorDataMgr::GetNpcTradingNum(bool bSell)
{
	int nNum = 0;
	for (auto it : m_uuid2NpcProtectorDataMap)
	{
		if (it.second && it.second.get() && 
			it.second.get()->status == MsgPB::PROTECTOR_STATUS::BADGE_STATUS_TRADE)
		{
			nNum++;  //先不区分售卖和购买的区别
			//auto&& pNpcType = NpcTypeCfg::getInstance().GetNpcType(it.second.get()->itemid);
			//if (pNpcType)
			//{
			//	if (bSell && pNpcType->n_CraftTpy == 0) {
			//		nNum++;
			//	}
			//	if (!bSell && pNpcType->n_CraftTpy == 1) {
			//		nNum++;
			//	}
			//}
		}
...
void NpcProtectorDataMgr::SetGuidPrice(int32 nItemId, int nCoin, int nCash)
{
	TradeGuidPrice data;
	data.nCoin = nCoin;
	data.nCash = nCash;
	m_mapGuidPrice[nItemId] = data;
}

bool NpcProtectorDataMgr::GetGuidPrice(int32 nItemId, int& nCoin, int& nCash)
{
	auto&& iter = m_mapGuidPrice.find(nItemId);
	if (iter != m_mapGuidPrice.end()) {
		nCoin = iter->second.nCoin;
		nCash = iter->second.nCash;
		return true;
	}
	return false;
}
