...
public:
	...
	NS::int32 comfort{}; // 舒適度等級 (星级 开启技能 );
	NS::I64 equipId{}; // 盔甲
	NS::I64 weaponId1{}; // 武器
...
struct ProtectorTradeData
{
	NS::I64 iggId{ 0 };
	NS::I64 uuid{ 0 };
	int32 itemId{ 0 };
	std::vector<ProtectorTrade> sellItems;
	std::vector<ProtectorTrade> buyItems;
};
...
public:
	...
	typedef std::vector<NpcTradeData> VecNpcTradeData;
	typedef std::map<NS::int32, TradeGuidPrice> MapTradeGuidPrice;
...
public:
	...
	bool isHasGeneralDead();

	bool isHasComfelRedByUuid(NS::I64 uuid);
	bool isHasComfelRed();
	...
	const ProtectorTradeData& GetProtectorTradeData() { return m_TradeProtectorData; }
	void updateProtectorTradeData(const MsgPB::MsgProtectorTradeInfoS2C& msg);
	void updateNpcTradeData(const MsgPB::MsgTradeNpcAppearance& msg);

	const NpcTradeData* GetTradeNpcData(int nIdx, bool bSell);
	int GetNpcTradingNum(bool bSell);

	void SetGuidPrice(int32 nItemId, int nCoin, int nCash);
	bool GetGuidPrice(int32 nItemId, int& nCoin, int& nCash);
private:
	...
	ProtectorTradeData m_TradeProtectorData;
	VecNpcTradeData	 m_vecSellsData;
	VecNpcTradeData  m_vecBuysData;
	MapTradeGuidPrice m_mapGuidPrice;
};
